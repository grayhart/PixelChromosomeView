...
"""
    Formats from difference DNA companies:
        Ancestry:
            RSID, Chromosome, Position, Allele1, Allele2
            I for Inserts, D for Deletions, 0 for no calls
        23andMe:
            RSID, Chromosome, Position, Alleles (allele1 and allele2 in a 2 character string)
            I for Inserts, D for deltions, - for no calls
        ftdna-illuminia 
            RSID, Chromosome, Position, Alleles (allele1 and allele2 in a 2 character string)
            I for Inserts, D for deltions, - for no calls
            NOTE: I need to see a real commercial version of this. The one I have seen is
            partially comma delimited and partially colon. It also seem inconsistent as to 
...
def get_chromsome_dataframe_for_matches(
    ...
    for known_relative in all_matches_list:
        ...
        if 'Ancestry' in kr_raw_file_name:
            ...
            if 'ftdna' in kr_raw_file_name:
                # FTDNA and 23andMe seem to have the same formats
                this_df = pd.read_csv(this_kr_raw_file,
                                        sep=',',
                                        skip_blank_lines=True,
                                        comment='#',
                                        header=0,
                                        names=['rsid', 'chromosome', 'position', "{0}_allele_pair".format(known_relative)])
            else:
                # FTDNA and 23andMe seem to have the same formats
                this_df = pd.read_csv(this_kr_raw_file,
                                        sep='\t',
            ...
            this_df["{0}-allele1".format(known_relative)] = this_df["{0}_allele_pair".format(known_relative)].str[0]
            this_df["{0}-allele2".format(known_relative)] = this_df["{0}_allele_pair".format(known_relative)].str[1]
            # this_df.drop(columns=['allele_pair'])
            this_df.drop(["{0}_allele_pair".format(known_relative)], axis=1, inplace=True)
        ...
        # Drop the rsid column as we don't use it and it duplicates with each merged kit
        this_df.drop(['rsid'], axis=1, inplace=True)
        ...
        if FILTER_NO_CALLS:
            ...
            # non Ancestry format
            this_df = this_df.drop(this_df[this_df["{0}-allele1".format(known_relative)] == '-'].index)
            this_df = this_df.drop(this_df[this_df["{0}-allele2".format(known_relative)] == '-'].index)
...
def calculate_segment_matches(
    ...
    for match_pair_combination in match_pair_combinations:
        ...
        red_df = chr_df[chr_df[mp_abbr_color] == NO_MATCH_SNP_COLOR]
        green_df = chr_df[chr_df[mp_abbr_color] == FULLY_IDENTICAL_SNP_COLOR]
...
def show_match_graphics(
    ...
    image_file = os.path.join(image_file_dir,"pixel_chr_{2}_{1}_chr_{0}.png".format(
